...
if df_moments is not None and df_conversations is not None and df_answers is not None:
            ...
            if not filtered_df.empty:
                ...
                if st.button("계산하기"):
                    try:
                        ...
                        # 임시 저장 버튼
                        if st.button("계산 결과 임시 저장"):
                            st.session_state.saved_results.append({
                                "Solution": solution_type,
                                "Plan": selected_plan,
                                "Details": f"MEP: {input_mep}, Usage: {expected_usage}",
                                "EUR": total_cost_eur,
                                "KRW": total_cost_krw,
    ...
    # --- 전체 금액 보기 로직 ---
    elif menu_type == "전체 금액 보기":
        st.header("전체 솔루션 예상 매입가")
        if st.session_state.saved_results:
            summary_df = pd.DataFrame(st.session_state.saved_results)
            st.table(summary_df.style.format({
                "EUR": "{:,.2f}",
                "KRW": "{:,.0f}",
                "Partner KRW": "{:,.0f}"
            }))
            if st.button("전체 결과 삭제"):
                st.session_state.saved_results = []
                st.info("저장된 모든 결과가 삭제되었습니다.")
                st.rerun()
        else:
